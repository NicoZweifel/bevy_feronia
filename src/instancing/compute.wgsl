...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let min_dist = camera.settings.x;
    let max_dist = camera.settings.y;

    let density = 1.0 - smoothstep(min_dist, max_dist, dist);
    if (hash_noise(instance.index) > density) {
        return;
    }
