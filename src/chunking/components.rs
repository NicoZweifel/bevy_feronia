...
impl ChunkConfig {
    ...
    pub fn get_chunk_world_center(&self, grid_coord: Vec3, level: u32) -> Vec3 {
        let world_size = self.get_chunk_world_size(level);
        let half_size = world_size / 2.0;

        grid_coord * world_size + Vec3::new(half_size, 0.0, half_size)
    }
    ...
    pub fn get_height_map_sampler(
        ...
        match height_map {
            ...
            Some(x) => match images.get(&x.0) {
                None => None,
                Some(img) => Some(HeightMapCpuSampler::new(img, self.get_total_world_size())),
            },
        }
    }
}
